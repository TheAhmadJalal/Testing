...
export const ElectionProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchElectionStats = async () => {
    ...
    try {
      ...
      // First fetch current election
      const electionResponse = await fetch(
        `${API_BASE_URL}/api/elections/current`
      );
      ...
      console.log("Current election fetched:", currentElection._id);
      console.info(
        `%c
        Project: School E-Voting System
        Developer: Hassan Iftikhar
      ...
      // Include election ID explicitly to ensure we get data for the current election
      const response = await fetch(
        `${API_BASE_URL}/api/elections/stats?electionId=${
          currentElection._id
        }&_t=${Date.now()}`
      );
      ...
      if (dataHash === lastDataHash) {
        ...
        console.log("Fetched election stats:", {
          totalVoters: data.totalVoters,
          votedCount: data.votedCount,
          electionId: data.electionId || "Not provided", // Log the election ID
        });
        console.info(
          `%c
          Project: School E-Voting System
  ...
  useEffect(() => {
    const fetchElectionStatus = async () => {
      try {
        ...
        // Use voting times consistently from the response
        if (data.votingStartDate && data.votingEndDate) {
          // Parse voting period into Date objects using UTC (Ghana/Accra time)
          const votingStart = parseDateTime(
            data.votingStartDate,
            data.votingStartTime || "08:00"
          );

          const votingEnd = parseDateTime(
            data.votingEndDate,
            data.votingEndTime || "17:00"
          );

          // Get current time as UTC timestamp (Ghana/Accra time)
          const nowUTC = Date.now();

          // Determine status based on timestamps
          let mappedStatus: "not-started" | "active" | "ended" = "not-started";

          if (
            nowUTC >= votingStart.getTime() &&
            nowUTC <= votingEnd.getTime()
          ) {
            mappedStatus = "active";
          } else if (nowUTC > votingEnd.getTime()) {
            mappedStatus = "ended";
          }

          setElectionStatus(mappedStatus);
        } else {
          // Fall back to the status field if voting dates aren't available
