...
import noneOptionImage from "../assets/NoneOfListed.png";
...
const Candidates: React.FC = () => {
  ...
  // This state is defined to store only ONE candidate ID per position
  ...
  useEffect(() => {
    ...
    // Check for location state indicating we just came from VotingAuth
    if (location.state) {
      const { voterName } = location.state as {
        voterName?: string;
      };
    ...
    const fetchCandidatesData = async (showLoading = true, useCache = true) => {
      ...
      // Create new abort controller with a timeout to prevent hanging requests
      ...
      try {
        ...
        if (useCache) {
          ...
          if (cachedData && now - cacheTimestamp < CACHE_EXPIRY_MS) {
            ...
            if (parsedData && Object.keys(parsedData).length > 0) {
              ...
              // Hide loading overlay after data is loaded from cache
              setShowLoadingOverlay(false);
        ...
        const response = await axios.get(
          ...
          {
            timeout: 8000, // Reduced timeout from 10s to 8s
            headers: {
        ...
        if (showLoading) {
          setLoading(false);
          // Add a short delay to make transition smoother
          setTimeout(() => {
            setShowLoadingOverlay(false);
          }, 300);
        } else {
  ...
  }, [user, navigate, apiUrl, location.state]);
  ...
  const handleVote = (candidate: Candidate, position: string) => {
    ...
    // This REPLACES the previous selection instead of adding to it:
  ...
  return (
    <div className="min-h-screen bg-indigo-50 relative" ref={topRef}>
      {/* Loading Overlay */}
      {showLoadingOverlay && (
        <div className="fixed inset-0 bg-indigo-900/70 backdrop-blur-sm z-[100] flex flex-col items-center justify-center">
          <div className="bg-white/10 p-8 rounded-xl shadow-2xl backdrop-blur-lg border border-white/20 max-w-md w-full">
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 border-t-4 border-b-4 border-indigo-500 rounded-full animate-spin mb-4"></div>
              <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">
                {voterName ? `Welcome, ${voterName}!` : "Loading Ballot"}
              </h2>
              <p className="text-indigo-200 text-center">
                Please wait while we prepare your voting ballot...
              </p>
            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                      {(Array.isArray(positionCandidates)
                        ...
                        return (
                          ...
                          >
                            <div className="relative">
                              <div className="aspect-[6/7] bg-white rounded-t-lg overflow-hidden flex items-center justify-center">
                                <img
                                  src={
                                    candidate.image ||
                                    candidate.imageUrl ||
                      ...
                      >
                        <div className="relative">
                          <div className="aspect-[6/7] bg-red-100 rounded-t-lg  overflow-hidden flex items-center justify-center ">
                            <img
                              src={noneOptionImage}
