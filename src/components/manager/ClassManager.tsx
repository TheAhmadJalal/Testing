...
const ClassManager: React.FC = () => {
  ...
  const canViewClass = hasPermission("class", "view");
    ...
    return (
      <div className="bg-yellow-50 p-6 rounded-lg text-center">
        ...
        <p className="text-yellow-700">
          You don't have permission to view class management.
        </p>
      </div>
    );
  }
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Classes - ${settings?.electionTitle || "Election"}</title>
          ${styles}
        </head>
        <body>
          <h1>Classes</h1>
  ...
  const handleExportExcel = () => {
    ...
    csvContent += "S/N,Class Name,Description,Status\n";
    ...
    link.setAttribute(
      "download",
      `${settings?.electionTitle || "election"}_classes_list_${
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 bg-gradient-to-r from-indigo-700 to-purple-700 text-white p-4 rounded-lg shadow-md">
        <div>
          ...
          <p className="text-indigo-100 text-sm font-sans font-light">
            Manage school classes for {currentElection?.title || "election"}
          </p>
        </div>
