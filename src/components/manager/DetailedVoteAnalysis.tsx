...
const DetailedVoteAnalysis: React.FC = () => {
  ...
  const { settings } = useSettings(); // Add this to access election title
    ...
    printWindow.document.write(`
      ...
      <html>
        ...
        <body>
          ...
          <h1>${settings?.electionTitle || "Election"}</h1>
          <h2>Detailed Vote Analysis</h2>
  ...
  const handleExportExcel = () => {
    ...
    csvContent += "Voter ID,Name,Class,Date/Time,Position,Voted For\n";
    ...
    link.setAttribute(
      "download",
      `${settings?.electionTitle || "election"}_detailed_vote_analysis_${
