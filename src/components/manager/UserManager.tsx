...
const UserManager: React.FC = () => {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add the new user to the list with properly formatted date fields
      const newUser = response.data.user;
      const formattedUser = {
        ...newUser,
        id: newUser._id || newUser.id,
        // Make sure the date fields are properly set in the local state
        activeFrom: formData.activeFrom || null,
        activeUntil: formData.activeUntil || null,
      };
  ...
  // Add the missing function to toggle a user's active status
  const handleToggleUserActive = async (user: UserType) => {
    try {
      ...
      setUsers(
        users.map((u) =>
          u.id === user.id ? { ...u, active: updatedStatus } : u
        )
      );
  ...
  // Format account duration for display
  const formatAccountDuration = (user: UserType) => {
    if (!user.activeFrom && !user.activeUntil) {
    ...
    if (user.activeFrom) {
      const fromDate = new Date(user.activeFrom);
      const fromDateStr = fromDate.toLocaleDateString();

      if (new Date() < fromDate) {
        durationText += `Activates on ${fromDateStr}`;
      } else {
        durationText += `Active since ${fromDateStr}`;
      }
    }

    if (user.activeUntil) {
      const untilDate = new Date(user.activeUntil);
      const untilDateStr = untilDate.toLocaleDateString();
      ...
      if (new Date() > untilDate) {
        durationText += `Expired on ${untilDateStr}`;
      } else {
        durationText += `Until ${untilDateStr}`;
      }
    }

    return durationText || "No time limit";
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="md:col-span-2 border-t pt-4 mt-2">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Set a date range for account activation. The account will
