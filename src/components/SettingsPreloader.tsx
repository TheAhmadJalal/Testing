...
const SettingsPreloader: React.FC<SettingsPreloaderProps> = ({ children }) => {
  ...
  const fetchSettings = async () => {
    ...
    if (loadingSettings) {
      console.log(
        "[SETTINGS] Already loading settings, skipping duplicate call"
      );
      return false;
    }
    ...
    try {
      ...
      try {
        localStorage.setItem("settings", JSON.stringify(cacheData));
      } catch (storageError) {
        console.warn("Failed to cache settings in localStorage:", storageError);
      }
  ...
  // Extract the cache loading to a separate function for reusability
  const loadCachedSettings = () => {
    try {
      ...
      console.error("[SETTINGS] Error parsing cached settings:", cacheError);
    }
    return false;
  };
